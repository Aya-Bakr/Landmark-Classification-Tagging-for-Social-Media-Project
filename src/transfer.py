...
def get_model_transfer_learning(model_name="resnet18", n_classes=50):
    ...
    elif hasattr(model_transfer, 'classifier') and isinstance(model_transfer.classifier, nn.Sequential):
        num_ftrs = model_transfer.classifier[-1].in_features
        model_transfer.classifier[-1] = nn.Linear(num_ftrs, n_classes)
    else:
        raise ValueError("The model architecture does not have a standard fully connected layer to modify.")
